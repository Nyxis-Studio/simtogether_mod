...
import os
import shutil
from settings import assets_path, mods_folder, project_name, build_path
from Utility.helpers_path import ensure_path_created, remove_file

mod_name_folder_path = mods_folder + os.sep + project_name
...
def copy_tl_packages(src: str, dest: str) -> int:
    ...
    for root, dirs, files in os.walk(src):
        for filename in fnmatch.filter(files, "*.package"):
            try:
                shutil.copy(root + os.sep + filename, dest + os.sep + filename)
                count += 1
            except Exception as e:
                file_list_failed.append(root + os.sep + filename)
                print("")
                print("Exception: " + str(e))
                print("Failed to copy file: " + root + os.sep + filename)
                print("To destination: " + dest + os.sep + filename)
                print("")
